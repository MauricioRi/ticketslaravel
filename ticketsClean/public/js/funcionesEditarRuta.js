...
$(document).on("click", ".buttoninsert", (e) => {
    ...
    if (optVal > 0) {
        ...
        const resultado = data.find(
            (data) => data.namegoefence === namegoefence
        );
        if (typeof resultado === "undefined") {
            ...
            data.push(feed); //lista de puntos
    ...
    for (var x = 0; x < puntos + 1; x++) {
        ...
        for (var y = 0; y < puntos + 1; y++) {
            if (x == 0 && y == 0) {
                matriz +=
                    "<td>" +
                    `<input disabled type="text" id="" name="" value="` +
                    "ORIGEN/DESTINO" +
                    `"></td>`;
            }
            ...
            if (x > 0 && y == 0) {
                matriz +=
                    "<td>" +
                    `<input disabled type="text" id="" name="" value="` +
                    data[x - 1].namegoefence +
                    `"></td>`;
            }
            if (x == y && x > 0 && y > 0) {
                // {{old(`+x + "," + y +`)}}
                const resultado = costos.find(
                    (busqueda) =>
                        busqueda.origen == data[x - 1].id &&
                        busqueda.destino == data[y - 1].id
                );

                const posicion = costos.findIndex(
                    (busqueda) =>
                        busqueda.origen == data[x - 1].id &&
                        busqueda.destino == data[y - 1].id
                );
                ...
                if (typeof resultado != "undefined") {
                    matriz +=
                        "<td>" +
                        `<input required type="number" class="matcost" id="" name="" value= "` +
                        costos[posicion].costo +
                        `" data-origen ="` +
                        data[x - 1].id +
                        `" data-destino="` +
                        data[y - 1].id +
                    ...
                    matriz +=
                        "<td>" +
                        `<input required type="number" class="matcost" id="" name="" value= "" data-origen ="` +
                        data[x - 1].id +
                        `" data-destino="` +
                        data[y - 1].id +
                        `"></td>`;
            ...
            if ((y > x) & (x > 0) && y > 0) {
                ...
                if (typeof resultado != "undefined") {
                    matriz +=
                        "<td>" +
                        `<input required class="matcost" type="number" id="" name="" value="` +
                        costos[posicion].costo +
                        `" data-origen ="` +
                        data[x - 1].id +
                        `" data-destino="` +
                    ...
                    matriz +=
                        "<td>" +
                        `<input required class="matcost" type="number" id="" name="" value="" data-origen ="` +
                        data[x - 1].id +
                        `" data-destino="` +
                        data[y - 1].id +
                        `"></td>`;
...
$(document).on("blur", ".matcost", (e) => {
    ...
    if (valor != "") {
        const resultado = costos.find(
            (data) => data.origen == origen && data.destino == destino
        );
        const posicion = costos.findIndex(
            (data) => data.origen == origen && data.destino == destino
        );
